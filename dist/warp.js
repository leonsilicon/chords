...
async function createCommand(chords) {
  ...
  const keybindings = exists(keybindingsPath) ? import_yaml.default.parse(readFileSync2(keybindingsPath, "utf8")) : {};
  for (const cmd of sortedCommands) {
    const keybind = syntheticKeybinds[cmd];
    keybindings[cmd] = keybind;
  }
  writeFileSync2(keybindingsPath, import_yaml.default.stringify(keybindings));
  const commandToKey = {};
